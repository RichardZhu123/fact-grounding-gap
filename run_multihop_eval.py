...
from simple_multihop_qa import SimpleMultiHopQA, Trajectory
...
def run_evaluation(
    ...
    for item in tqdm(data, desc="Running multi-hop QA"):
        ...
        ans_obj = item.get("answers_objects", [{}])
        gold_answer = ans_obj[0].get("spans", [""])[0] if ans_obj else item.get("answer", "")
