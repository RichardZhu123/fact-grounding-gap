...
if os.path.exists(probs_file):
    ...
    probs_list = []  # list of [prob_answerable, true_label]
    batch_size = 8  # Small batches to avoid OOM
    ...
    for i in range(0, len(dev_examples), batch_size):
        ...
        for ex in batch:
            ...
            texts.append(f"{sq} [SEP] {ctx[:1024]}")
            labels.append(1 if ex['label'] == 'ANSWERABLE' else 0)
        
        inputs = tokenizer(texts, padding=True, truncation=True, max_length=512, return_tensors='pt')
        # Cast input_ids to long (required), keep on CPU
        with torch.no_grad():
            logits = model(**inputs).logits.float()  # back to float32 for softmax
...
print()
print("Note: Flag% here is on the dev classification task (per-hop).")
print("In intervention experiments, 50.0% of hops were flagged at threshold=0.5.")
print("Threshold 0.5 is the operating point used in all reported experiments.")
