...
def analyze(name, path, deberta_key='deberta', always_key='always'):
    with open(path) as f:
        d = json.load(f)
    ...
    deberta_flagged = sum(sum(ex[deberta_key]['flags']) for ex in examples)
    
    base_correct = sum(1 for ex in examples if ex['baseline']['correct'])
    always_correct = sum(1 for ex in examples if ex[always_key]['correct'])
    deberta_correct = sum(1 for ex in examples if ex[deberta_key]['correct'])
...
aug = analyze("MUSIQUE AUGMENTATION", "results/section4_full_dev.json",
              deberta_key='deberta', always_key='always')
rerank = analyze("MUSIQUE RERANKING", "results/section4_rerank_full.json",
                 deberta_key='deberta_rerank', always_key='always_rerank')
...
if 'deberta' in ex0:
    ...
    # Find the deberta key
    dkey = [k for k in ex0 if 'deberta' in k][0]
    akey = [k for k in ex0 if 'always' in k][0]
    print(f"  (detected keys: {dkey}, {akey})")

total_hops_hqa = sum(ex['n_hops'] for ex in examples_hqa)
deberta_flagged_hqa = sum(sum(ex[dkey]['flags']) for ex in examples_hqa)

base_correct_hqa = sum(1 for ex in examples_hqa if ex['baseline']['correct'])
always_correct_hqa = sum(1 for ex in examples_hqa if ex[akey]['correct'])
deberta_correct_hqa = sum(1 for ex in examples_hqa if ex[dkey]['correct'])
